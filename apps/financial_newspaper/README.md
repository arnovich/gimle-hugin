...
When `HUGIN_CTRLRTN=1`, provider calls carry the Hugin session ID in
both `x-ctrlrtn-task` (edition outcomes) and `x-ctrlrtn-session` (lifetime spend
limits), with `x-ctrlrtn-route` identifying the agent role. The session reports
