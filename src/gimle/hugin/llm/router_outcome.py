"""Per-edition outcome report for ctrlrtn.

Companion to ``router_correlation.py``: where that stamps ``x-ctrlrtn-task`` on an
...
boundary, to the router's control-plane endpoint ``POST {base}/ctrlrtn/outcome``
...
Opt-in and best-effort, mirroring the header:
  - Only sent when ``HUGIN_CTRLRTN`` is truthy (the same flag that enables
  ...
  - Target base URL: the outcome endpoint lives on the SAME router the model
    ...
    set. ``CTRLRTN_URL`` is only an override for the rare case where the
...
_URL_ENVS = ("CTRLRTN_URL", "ANTHROPIC_BASE_URL", "OPENAI_BASE_URL")
...
def report_outcome(
    ...
    *,
    ...
    """Report one edition's result to ctrlrtn.
    ...
    try:
        ...
        logger.warning(
            "ctrlrtn: failed to report outcome for task %s: %s",
            task_id,
            exc,
        )
        return False
